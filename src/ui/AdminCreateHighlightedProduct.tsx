...
const AdminCreateHighlightedProduct = ({setAddhighlightedProductModal}) => {
    ...
    const handleCreatingHighlighProduct = async (e: any) => {
        e.preventDefault();
        const formData = new FormData(e.target);
        const productData: any = Object.fromEntries(formData);
      ...
      const handleCancel = () => {
        setAddhighlightedProductModal(false);
      };
    
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
    <div className="bg-gray-950 rounded-lg w-full max-w-5xl mx-4 sm:mx-auto">
      <form onSubmit={handleCreatingHighlighProduct} className=" py-10 px-6 sm:px-10 lg:px-16 text-white max-h-[90vh] overflow-y-auto">
        <button
        ...
        <div className="border-b border-b-white/10 pb-5">
          ...
          <p className="mt-1 text-sm leading-6 text-gray-400">
            Provide the product details to create a new highlight product.
          </p>
        </div>
        ...
        >
          {loading ? 'Loading...' : 'Create Highlited Product'}
        </button>
      </form>
    </div>
  </div>
